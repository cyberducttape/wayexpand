...
| Sway | stdin + libei; evdev only with explicit opt-in | evdev + libei; input-method-v2 only when probed | wlroots scaffold, not active tracking | Implemented, awaiting compositor certification |
| Hyprland | stdin + libei; evdev only with explicit opt-in | evdev + libei; input-method-v2 only when probed | wlroots scaffold, not active tracking | Implemented, awaiting compositor certification |
| river | stdin + libei; evdev only with explicit opt-in | evdev + libei; input-method-v2 only when probed | wlroots scaffold, not active tracking | Implemented, awaiting compositor certification |
...
`doctor` probe is required before describing input-method-v2 or wlroots window
tracking as available on a particular compositor.
